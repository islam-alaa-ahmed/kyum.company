...
function currentRequestCandidates(){const map=new Map();rows.filter(r=>r.isCurrentUserSelection===true&&!r.completedAt&&!['مكتمل','بانتظار التأكيد','مؤكدة','ملغاة','ملغي'].includes(normalizeStatus(r.visitStatus||r.status))).forEach(r=>{const key=r.scheduleEntryId||r.id;if(!map.has(key))map.set(key,r)});return [...map.values()].sort((a,b)=>new Date(b.selectedForExecutionAt||0)-new Date(a.selectedForExecutionAt||0))}
