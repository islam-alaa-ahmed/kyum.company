...
function currentRequestCandidates(){const map=new Map();rows.filter(isActive).forEach(r=>{const key=r.scheduleEntryId||r.id,current=map.get(key);if(!current||r.isCurrentUserSelection||new Date(r.selectedForExecutionAt||r.startedAt||r.onRouteAt||0)>new Date(current.selectedForExecutionAt||current.startedAt||current.onRouteAt||0))map.set(key,r)});return [...map.values()].sort((a,b)=>new Date(b.selectedForExecutionAt||b.startedAt||b.onRouteAt||0)-new Date(a.selectedForExecutionAt||a.startedAt||a.onRouteAt||0))}
