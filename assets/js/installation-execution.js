...
function isExecutionClosed(r){
  return Boolean(r.completedAt)||['مكتمل','بانتظار التأكيد','مؤكدة','ملغاة','ملغي'].includes(normalizeStatus(r.visitStatus||r.status));
}
function isExecutionInProgressRow(r){
  return !isExecutionClosed(r)&&Boolean(r.hasOperationalProgress||hasExecutionProgress(r));
}
function isPendingExecutionRow(r){
  return !isExecutionClosed(r)&&!isExecutionInProgressRow(r);
}
...
function filteredToday(){
  ...
  return rows.filter(r=>{
    if(isExecutionClosed(r)||!r.scheduledDate)return false;
    const pending=isPendingExecutionRow(r),inProgress=isExecutionInProgressRow(r);
    const dateMatches=includeOverdue
      ? r.scheduledDate<=date&&(pending||inProgress)
      : r.scheduledDate===date&&(pending||inProgress);
...
function currentRequestCandidates(){
  const map=new Map();
  rows.filter(r=>!isExecutionClosed(r)&&(r.isCurrentUserSelection===true||isExecutionInProgressRow(r))).forEach(r=>{
    const key=r.scheduleEntryId||r.id;if(!map.has(key))map.set(key,r);
  });
  return [...map.values()].sort((a,b)=>{
    const progressA=new Date(a.startedAt||a.arrivedAt||a.mapOpenedAt||a.onRouteAt||a.selectedForExecutionAt||0).getTime();
    const progressB=new Date(b.startedAt||b.arrivedAt||b.mapOpenedAt||b.onRouteAt||b.selectedForExecutionAt||0).getTime();
