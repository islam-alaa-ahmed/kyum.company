(function(){
  ...
  async function installationCostWorkspace(year,month){
    ...
    const [{data:technicians,error:techError},{data:categories,error:categoryError},{data:annual,error:annualError},{data:monthly,error:monthlyError},{data:assignments,error:assignmentError},{data:teams,error:teamError}]=await Promise.all([
      db().rpc('get_installation_cost_technicians'),
      ...
      db().from('installation_technician_annual_costs').select('id,fiscal_year,technician_name,category_id,annual_total').eq('fiscal_year',Number(year)),
      db().from('installation_technician_monthly_costs').select('id,cost_month,technician_name,category_id,amount,is_override').eq('cost_month',costMonth),
      db().from('installation_cost_team_assignments').select('id,cost_month,technician_name,installation_team_id').eq('cost_month',costMonth),
      db().from('installation_teams').select('id,name,status').neq('status','غير نشطة').order('name')
    ]);
    const err=techError||categoryError||annualError||monthlyError||assignmentError||teamError;if(err)throw new Error('تعذر تحميل تكلفة قسم التركيبات: '+err.message);
    return {year:Number(year),month:Number(month),costMonth,technicians:(technicians||[]).map(x=>x.name).filter(Boolean),categories:categories||[],annual:annual||[],monthly:monthly||[],assignments:assignments||[],teams:teams||[]};
  }
  async function saveInstallationCostAnnual(payload){
    requireAction('edit','installationCosts');
    const record={fiscal_year:Number(payload.year),technician_name:String(payload.technicianName||'').trim(),category_id:payload.categoryId,annual_total:Number(payload.annualTotal||0),updated_at:new Date().toISOString()};
    if(!record.technician_name||!record.category_id)throw new Error('بيانات الفني أو بند التكلفة غير مكتملة.');
    const {error}=await db().from('installation_technician_annual_costs').upsert(record,{onConflict:'fiscal_year,technician_name,category_id'});if(error)throw new Error('تعذر حفظ إجمالي التكلفة: '+error.message);
  }
  async function saveInstallationCostMonth(payload){
    requireAction('edit','installationCosts');
    const costMonth=installationCostMonthKey(payload.year,payload.month),record={cost_month:costMonth,technician_name:String(payload.technicianName||'').trim(),category_id:payload.categoryId,amount:Number(payload.amount||0),is_override:true,updated_at:new Date().toISOString()};
    if(!record.technician_name||!record.category_id)throw new Error('بيانات الفني أو بند التكلفة غير مكتملة.');
    const {error}=await db().from('installation_technician_monthly_costs').upsert(record,{onConflict:'cost_month,technician_name,category_id'});if(error)throw new Error('تعذر حفظ تكلفة الشهر: '+error.message);
  }
  async function clearInstallationCostMonth(payload){
    requireAction('edit','installationCosts');const costMonth=installationCostMonthKey(payload.year,payload.month);let q=db().from('installation_technician_monthly_costs').delete().eq('cost_month',costMonth).eq('technician_name',String(payload.technicianName||'').trim());if(payload.categoryId)q=q.eq('category_id',payload.categoryId);const {error}=await q;if(error)throw new Error('تعذر استعادة تكلفة الشهر من الإجمالي: '+error.message);
  }
  async function saveInstallationCostTeamAssignment(payload){
    requireAction('edit','installationCosts');const record={cost_month:installationCostMonthKey(payload.year,payload.month),technician_name:String(payload.technicianName||'').trim(),installation_team_id:payload.teamId||null,updated_at:new Date().toISOString()};if(!record.technician_name)throw new Error('اسم الفني غير متاح.');const {error}=await db().from('installation_cost_team_assignments').upsert(record,{onConflict:'cost_month,technician_name'});if(error)throw new Error('تعذر حفظ توزيع الفني على الفرقة: '+error.message);
  }
  async function saveInstallationCostCategory(payload){
    requireAction(payload.id?'edit':'add','installationCosts');const record={name:String(payload.name||'').trim(),sort_order:Number(payload.sortOrder||100),is_active:true,updated_at:new Date().toISOString()};if(!record.name)throw new Error('اكتب اسم بند التكلفة.');const q=payload.id?db().from('installation_cost_categories').update(record).eq('id',payload.id):db().from('installation_cost_categories').insert(record);const {error}=await q;if(error)throw new Error('تعذر حفظ بند التكلفة: '+error.message);
  }
  ...
  async function copyPreviousInstallationCostMonth(year,month){
    requireAction('edit','installationCosts');const target=new Date(Number(year),Number(month)-1,1),prev=new Date(target);prev.setMonth(prev.getMonth()-1);const targetKey=installationCostMonthKey(target.getFullYear(),target.getMonth()+1),prevKey=installationCostMonthKey(prev.getFullYear(),prev.getMonth()+1);const [{data:monthly,error:monthlyError},{data:assignments,error:assignmentError}]=await Promise.all([db().from('installation_technician_monthly_costs').select('technician_name,category_id,amount,is_override').eq('cost_month',prevKey),db().from('installation_cost_team_assignments').select('technician_name,installation_team_id').eq('cost_month',prevKey)]);if(monthlyError||assignmentError)throw new Error('تعذر تحميل بيانات الشهر السابق: '+(monthlyError||assignmentError).message);if((monthly||[]).length){const {error}=await db().from('installation_technician_monthly_costs').upsert(monthly.map(x=>({cost_month:targetKey,technician_name:x.technician_name,category_id:x.category_id,amount:Number(x.amount||0),is_override:true,updated_at:new Date().toISOString()})),{onConflict:'cost_month,technician_name,category_id'});if(error)throw new Error('تعذر نسخ تكاليف الشهر السابق: '+error.message)}if((assignments||[]).length){const {error}=await db().from('installation_cost_team_assignments').upsert(assignments.map(x=>({cost_month:targetKey,technician_name:x.technician_name,installation_team_id:x.installation_team_id,updated_at:new Date().toISOString()})),{onConflict:'cost_month,technician_name'});if(error)throw new Error('تعذر نسخ توزيع الفرق: '+error.message)}return {monthly:(monthly||[]).length,assignments:(assignments||[]).length};
  }
