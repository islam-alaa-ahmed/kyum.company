(function(){
  ...
  function setSummaryDate(value){const x=new Date(`${value}T12:00:00`),year=$('installationSummaryYear'),month=$('installationSummaryMonth'),day=$('installationSummaryDay');if(!year||!month||!day)return;year.value=String(x.getFullYear());month.value=String(x.getMonth()+1);fillSummaryDays();day.value=String(x.getDate());document.querySelectorAll('.installation-summary-day-nav button').forEach(btn=>btn.classList.remove('is-active'));if(value===isoDate(new Date()))$('installationSummaryToday')?.classList.add('is-active')}
