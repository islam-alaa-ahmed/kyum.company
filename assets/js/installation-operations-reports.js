(function(){
  ...
  const state={exceptions:[],technicians:[],reportRows:[],summaryData:null,summaryTeams:[],summarySelectedTeams:new Set()};
  ...
  function setSummaryDate(value){const x=new Date(`${value}T12:00:00`);$('installationSummaryYear').value=String(x.getFullYear());fillSummaryDays();$('installationSummaryMonth').value=String(x.getMonth()+1);fillSummaryDays();$('installationSummaryDay').value=String(x.getDate())}
  ...
  function renderSummaryTeamOptions(){const host=$('installationSummaryTeamsOptions');if(!host)return;host.innerHTML=state.summaryTeams.map(t=>`<label><input type="checkbox" value="${esc(t.id)}" ${state.summarySelectedTeams.has(String(t.id))?'checked':''}><span>${esc(t.name)}</span></label>`).join('');updateSummaryTeamButton()}
  function updateSummaryTeamButton(){const btn=$('installationSummaryTeamsButton');if(!btn)return;const count=state.summarySelectedTeams.size,total=state.summaryTeams.length;btn.textContent=!count||count===total?'كل الفرق':`${count} فرق مختارة`}
  function fillSummarySelectors(data){const rep=$('installationSummaryRepresentative'),current=rep?.value||'';if(rep){rep.innerHTML='<option value="">كل المندوبين</option>'+data.representatives.map(x=>`<option value="${esc(x.id)}">${esc(x.name)}</option>`).join('');rep.value=current}const incoming=data.teams||[];const first=!state.summaryTeams.length;state.summaryTeams=incoming;if(first)state.summarySelectedTeams=new Set(incoming.map(x=>String(x.id)));else{const allowed=new Set(incoming.map(x=>String(x.id)));state.summarySelectedTeams=new Set([...state.summarySelectedTeams].filter(x=>allowed.has(x)));if(!state.summarySelectedTeams.size)state.summarySelectedTeams=new Set(incoming.map(x=>String(x.id)))}renderSummaryTeamOptions()}
  async function loadInstallationSummary(){const box=$('installationSummaryStatus');status(box,'جاري إعداد ملخص التركيبات...');try{const data=await window.InstallationsServiceSafe.installationSummaryReport({date:summaryDate(),representativeId:$('installationSummaryRepresentative')?.value||'',teamIds:[...state.summarySelectedTeams]});state.summaryData=data;fillSummarySelectors(data);renderInstallationSummary(data);status(box,'')}catch(e){status(box,e.message||'تعذر إعداد ملخص التركيبات.','error')}}
  function renderInstallationSummary(data){const s=data.summary;$('installationSummaryKpiTeams').textContent=num(s.teams);$('installationSummaryKpiVisits').textContent=num(s.visits);$('installationSummaryKpiQuantity').textContent=num(s.quantity);$('installationSummaryKpiValue').textContent=money(s.value);$('installationSummaryKpiAverage').textContent=money(s.average);const body=$('installationSummaryReportBody');if(!data.rows.length){body.innerHTML=empty(5);return}let html='';for(const team of data.rows){const span=team.services.length+1;team.services.forEach((svc,i)=>{html+=`<tr>${i===0?`<td class="installation-summary-team-cell" rowspan="${span}">${esc(team.name)}</td>`:''}<td>${esc(svc.name)}</td><td class="installation-summary-value">${num(svc.quantity)}</td><td class="installation-summary-value">${money(svc.value)}</td><td class="installation-summary-value">${money(svc.average)}</td></tr>`});html+=`<tr class="installation-summary-service-total"><td>إجمالي ${esc(team.name)}</td><td class="installation-summary-value">${num(team.quantity)}</td><td class="installation-summary-value">${money(team.value)}</td><td class="installation-summary-value">${money(team.average)}</td></tr>`}html+=`<tr class="installation-summary-grand-total"><td colspan="2">الإجمالي العام</td><td class="installation-summary-value">${num(s.quantity)}</td><td class="installation-summary-value">${money(s.value)}</td><td class="installation-summary-value">${money(s.average)}</td></tr>`;body.innerHTML=html;const date=fmt(summaryDate()),rep=$('installationSummaryRepresentative')?.selectedOptions?.[0]?.textContent||'كل المندوبين';$('installationSummaryPrintFilters').textContent=`التاريخ: ${date} — المندوب: ${rep} — الفرق: ${state.summarySelectedTeams.size||state.summaryTeams.length}`}
  ...
  function bind(){
    ...
    ['installationSummaryYear','installationSummaryMonth','installationSummaryDay','installationSummaryRepresentative'].forEach(id=>$(id)?.addEventListener('change',()=>{if(id!=='installationSummaryDay')fillSummaryDays();loadInstallationSummary()}));
    $('installationSummaryPreviousDay')?.addEventListener('click',()=>{const d=new Date(summaryDate()+'T12:00:00');d.setDate(d.getDate()-1);setSummaryDate(isoDate(d));loadInstallationSummary()});
    $('installationSummaryToday')?.addEventListener('click',()=>{setSummaryDate(isoDate(new Date()));loadInstallationSummary()});
    $('installationSummaryNextDay')?.addEventListener('click',()=>{const d=new Date(summaryDate()+'T12:00:00');d.setDate(d.getDate()+1);setSummaryDate(isoDate(d));loadInstallationSummary()});
    ...
    $('installationSummaryTeamsOptions')?.addEventListener('change',e=>{if(!e.target.matches('input[type=checkbox]'))return;e.target.checked?state.summarySelectedTeams.add(e.target.value):state.summarySelectedTeams.delete(e.target.value);updateSummaryTeamButton();loadInstallationSummary()});
    $('installationSummaryTeamsMenu')?.addEventListener('click',e=>{const action=e.target.closest('[data-summary-teams]')?.dataset.summaryTeams;if(!action)return;state.summarySelectedTeams=action==='all'?new Set(state.summaryTeams.map(x=>String(x.id))):new Set();renderSummaryTeamOptions();loadInstallationSummary()});
    $('resetInstallationSummaryFilters')?.addEventListener('click',()=>{setSummaryDate(isoDate(new Date()));$('installationSummaryRepresentative').value='';state.summarySelectedTeams=new Set(state.summaryTeams.map(x=>String(x.id)));renderSummaryTeamOptions();loadInstallationSummary()});
