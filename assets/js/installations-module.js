(() => {
  ...
  async function openServicesEdit(row){if(!row)return;await ensureOptions();$("installationServicesEditRequestId").value=row.id;$("installationServicesEditLabel").textContent=`${row.requestNumber} — ${row.customerName}`;$("installationServicesEditDistrict").textContent=[row.city,row.district].filter(Boolean).join(' - ')||row.installationAddress||'—';const locationNode=$("installationServicesEditLocation");if(row.customerMapUrl){locationNode.innerHTML=`<a class="installation-context-location-link" href="${esc(row.customerMapUrl)}" target="_blank" rel="noopener noreferrer">فتح الموقع</a>`}else{locationNode.textContent=row.installationAddress||'—'}$("installationServicesEditCustomerOrder").textContent=row.customerOrderNumber||'—';$("installationServicesEditQuotation").textContent=row.quotationNumber||'بدون عرض سعر';$("installationServicesEditBody").innerHTML='';(row.services?.length?row.services:[{}]).forEach(addInlineServiceRow);clearStatus($("installationServicesEditStatus"));$("installationServicesEditDialog").showModal()}
