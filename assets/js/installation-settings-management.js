...
let cache={services:[],teams:[],neighborhoods:[]};
...
async function load(){message('جاري تحميل إعدادات التركيبات...');try{cache=await window.InstallationsService.settingsCatalog();render();message('')}catch(e){message(e.message||'تعذر تحميل الإعدادات.','error')}}
...
return `<label>اسم الحي<input name="name" required maxlength="120" value="${esc(row.name||'')}"></label><label>المدينة<input name="city" maxlength="120" value="${esc(row.city||'')}"></label><label>المنطقة<input name="region" maxlength="120" value="${esc(row.region||'')}"></label><label>الحالة<select name="isActive"><option value="1" ${row.is_active!==false?'selected':''}>نشط</option><option value="0" ${row.is_active===false?'selected':''}>متوقف</option></select></label>`}
function open(type,id=''){const list=type==='service'?cache.services:type==='team'?cache.teams:cache.neighborhoods,row=list.find(x=>x.id===id)||{};$('installationReferenceType').value=type;$('installationReferenceId').value=id;$('installationReferenceDialogTitle').textContent=(id?'تعديل ':'إضافة ')+(type==='service'?'خدمة':type==='team'?'فرقة تركيب':'حي');$('installationReferenceFields').innerHTML=fields(type,row);$('installationReferenceFormStatus').classList.add('hidden');$('installationReferenceDialog').showModal()}
async function submit(e){e.preventDefault();const fd=new FormData(e.currentTarget),type=$('installationReferenceType').value,payload=Object.fromEntries(fd.entries());payload.id=$('installationReferenceId').value;payload.isActive=payload.isActive!=='0';try{await window.InstallationsService.saveSettingItem(type,payload);$('installationReferenceDialog').close();await load();message('تم حفظ البيانات بنجاح.','success')}catch(err){const el=$('installationReferenceFormStatus');el.textContent=err.message;el.classList.remove('hidden');el.dataset.type='error'}}
function bind(){
  ...
  document.addEventListener('click',async e=>{const edit=e.target.closest('[data-setting-edit]');if(edit)return open(edit.dataset.settingEdit,edit.dataset.id);const toggle=e.target.closest('[data-setting-toggle]');if(toggle){try{await window.InstallationsService.toggleSettingItem(toggle.dataset.settingToggle,toggle.dataset.id,toggle.dataset.active!=='1');await load()}catch(err){message(err.message,'error')}return}const del=e.target.closest('[data-setting-delete]');if(del&&confirm('هل تريد حذف هذا السجل؟')){try{await window.InstallationsService.removeSettingItem(del.dataset.settingDelete,del.dataset.id);await load()}catch(err){message(err.message,'error')}}});
