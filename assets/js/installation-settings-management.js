...
const regionOptions=cache.regions.map(x=>`<option value="${esc(x.id)}" ${String(row.region_id||'')===String(x.id)?'selected':''}>${esc(x.name)}</option>`).join('');
const cityOptions=cache.cities.filter(x=>!row.region_id||String(x.region_id)===String(row.region_id)).map(x=>`<option value="${esc(x.id)}" data-region-id="${esc(x.region_id)}" ${String(row.city_id||'')===String(x.id)?'selected':''}>${esc(x.name)}</option>`).join('');
return `<label>اسم الحي<input name="name" required maxlength="120" value="${esc(row.name||'')}"></label><label>المنطقة<select name="regionId" id="installationReferenceRegion" required><option value="">اختر المنطقة</option>${regionOptions}</select></label><label>المدينة<select name="cityId" id="installationReferenceCity" required><option value="">اختر المدينة</option>${cityOptions}</select></label><label>الحالة<select name="isActive"><option value="1" ${row.is_active!==false?'selected':''}>نشط</option><option value="0" ${row.is_active===false?'selected':''}>متوقف</option></select></label>`}
function open(type,id=''){const list=type==='service'?cache.services:type==='team'?cache.teams:cache.neighborhoods,row=list.find(x=>x.id===id)||{};$('installationReferenceType').value=type;$('installationReferenceId').value=id;$('installationReferenceDialogTitle').textContent=(id?'تعديل ':'إضافة ')+(type==='service'?'خدمة':type==='team'?'فرقة تركيب':'حي');$('installationReferenceFields').innerHTML=fields(type,row);if(type==='neighborhood'){const region=$('installationReferenceRegion'),city=$('installationReferenceCity');region?.addEventListener('change',()=>{const selected=region.value;city.innerHTML='<option value="">اختر المدينة</option>'+cache.cities.filter(x=>String(x.region_id)===String(selected)).map(x=>`<option value="${esc(x.id)}">${esc(x.name)}</option>`).join('')})}$('installationReferenceFormStatus').classList.add('hidden');$('installationReferenceDialog').showModal()}
async function submit(e){e.preventDefault();const fd=new FormData(e.currentTarget),type=$('installationReferenceType').value,payload=Object.fromEntries(fd.entries());payload.id=$('installationReferenceId').value;payload.isActive=payload.isActive!=='0';if(type==='neighborhood'){const region=cache.regions.find(x=>String(x.id)===String(payload.regionId)),city=cache.cities.find(x=>String(x.id)===String(payload.cityId));payload.region=region?.name||'';payload.city=city?.name||''}try{await window.InstallationsServiceSafe.saveSettingItem(type,payload);$('installationReferenceDialog').close();await load();message('تم حفظ البيانات بنجاح.','success')}catch(err){const el=$('installationReferenceFormStatus');el.textContent=err.message;el.classList.remove('hidden');el.dataset.type='error'}}
function bind(){
  ...
  $('closeInstallationReferenceDialog')?.addEventListener('click',()=>$('installationReferenceDialog').close());
  $('cancelInstallationReferenceDialog')?.addEventListener('click',()=>$('installationReferenceDialog').close());
  document.addEventListener('click',async e=>{const edit=e.target.closest('[data-setting-edit]');if(edit)return open(edit.dataset.settingEdit,edit.dataset.id);const toggle=e.target.closest('[data-setting-toggle]');if(toggle){try{await window.InstallationsServiceSafe.toggleSettingItem(toggle.dataset.settingToggle,toggle.dataset.id,toggle.dataset.active!=='1');await load()}catch(err){message(err.message,'error')}return}const del=e.target.closest('[data-setting-delete]');if(del&&confirm('هل تريد حذف هذا السجل؟')){try{await window.InstallationsServiceSafe.removeSettingItem(del.dataset.settingDelete,del.dataset.id);await load()}catch(err){message(err.message,'error')}}});
