...
const views = {
  ...
  installationCosts: document.getElementById("installationCostsView"),
  ...
  advertisingDashboard: document.getElementById("advertisingDashboardView"),
  advertisingProjects: document.getElementById("advertisingProjectsView"),
  advertisingMaterialIssue: document.getElementById("advertisingMaterialIssueView"),
  advertisingInventory: document.getElementById("advertisingInventoryView"),
  advertisingCustodyPurchases: document.getElementById("advertisingCustodyPurchasesView"),
  advertisingProjectCosts: document.getElementById("advertisingProjectCostsView"),
  advertisingReports: document.getElementById("advertisingReportsView"),
  advertisingReferenceData: document.getElementById("advertisingReferenceDataView"),
...
const pageMeta = {
  ...
  installationCosts: ["تكلفة قسم التركيبات", "إدارة تكلفة الفنيين والفرق والتكاليف الشهرية"],
  ...
  advertisingDashboard: ["لوحة متابعة قسم الدعاية والإعلان", "ملخص المشاريع والتكلفة والربحية ومؤشرات تشغيل القسم"],
  advertisingProjects: ["المشاريع", "إدارة مشاريع قسم الدعاية والإعلان ومراحلها التشغيلية"],
  advertisingMaterialIssue: ["صرف المواد", "صرف المواد على المشاريع المفتوحة وتتبع تكلفة الحركة"],
  advertisingInventory: ["المخزون", "متابعة الأصناف والأرصدة وحركات ومتوسط تكلفة المخزون"],
  advertisingCustodyPurchases: ["العهد والمشتريات", "إدارة عهد الموظفين والمشتريات والتسويات"],
  advertisingProjectCosts: ["تكلفة وربحية المشاريع", "متابعة التكلفة الفعلية والربح والخسارة لكل مشروع"],
  advertisingReports: ["تقارير الدعاية والإعلان", "تحليلات المشاريع والمخزون والمشتريات والعهد والربحية"],
  advertisingReferenceData: ["البيانات المرجعية للدعاية والإعلان", "إدارة الأصناف والموظفين وأنواع المشاريع والبيانات المرجعية للقسم"],
...
let customerGeoController = null;
...
function positionCustomerGeoOptions(type){
  const controller=customerGeoController;
  const {search,options}=controller?.elements(type)||{};
  const dialog=document.getElementById('customerDialog');
  if(!search||!options||!dialog)return;
  if(window.matchMedia('(max-width: 767px)').matches){resetGeoOptionsPosition(options);return}
  ...
  const gap=8,minUsefulHeight=160,preferredHeight=280;
...
function ensureCustomerGeoController(){
  ...
  customerGeoController=window.KYUMGeography.createController({
    ids:{
      ...
      city:{wrapper:'customerCityCombobox',hidden:'customerCity',search:'customerCitySearch',options:'customerCityOptions'},
      district:{wrapper:'customerDistrictCombobox',hidden:'customerDistrict',search:'customerDistrictSearch',options:'customerDistrictOptions'}
    },
    optionLimit:250,
    boundAttribute:'customerGeoUnifiedBound',
    onOpen:type=>requestAnimationFrame(()=>positionCustomerGeoOptions(type)),
    onClose:(_type,{options})=>resetGeoOptionsPosition(options)
  }).bind();
  const customerDialog=document.getElementById('customerDialog');
  if(customerDialog&&!customerDialog.dataset.geoUnifiedScrollBound){
    customerDialog.dataset.geoUnifiedScrollBound='1';
    const closeAll=()=>['region','city','district'].forEach(type=>customerGeoController?.close(type));
    customerDialog.addEventListener('scroll',closeAll,{passive:true});
    customerDialog.querySelector('form')?.addEventListener('scroll',closeAll,{passive:true});
    customerDialog.addEventListener('close',closeAll);
    window.addEventListener('resize',closeAll,{passive:true});
  }
  return customerGeoController;
}
...
async function loadCustomerDistrictCatalog(force=false){
  ...
  customerDistrictCatalogPromise=(async()=>{
    ...
    customerCityCatalog=geo.cities;
    customerDistrictCatalog=geo.districts;
...
function applyDesktopHeaderStyle(value, { persist = true } = {}) {
  const style = normalizeDesktopHeaderStyle(value);
  document.documentElement.dataset.desktopHeaderStyle = style;
  if (persist) {
    try { localStorage.setItem(DESKTOP_HEADER_STYLE_STORAGE_KEY, style); } catch {}
  }
  const select = document.getElementById("systemDesktopHeaderStyle");
  if (select && select.value !== style) select.value = style;
  return style;
}

// Apply the last confirmed choice before System Settings is opened.
// The CSS owner is desktop-only, so this never changes the mobile header.
try {
