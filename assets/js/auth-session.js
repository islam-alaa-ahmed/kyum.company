(function () {
  ...
  const AUTH_CACHE_PREFIX = "kyum_offline_auth_v1:";

  function authCacheKey(userId) { return `${AUTH_CACHE_PREFIX}${userId}`; }
  function readCachedProfile(userId) {
    try {
      const value = JSON.parse(localStorage.getItem(authCacheKey(userId)) || "null");
      return value?.profile?.id === userId ? value.profile : null;
    } catch (_) { return null; }
  }
  function writeCachedProfile(userId, profile) {
    try {
      const previous = JSON.parse(localStorage.getItem(authCacheKey(userId)) || "null") || {};
      localStorage.setItem(authCacheKey(userId), JSON.stringify({ ...previous, profile, updatedAt: Date.now() }));
    } catch (_) {}
  }
  const el = () => ({
  ...
  async function loadProfile(userId) {
    ...
    try {
      ...
      throw new Error(`تعذر تحميل ملف المستخدم: ${error?.message || error}`);
    }
  }
  async function activate(session) {
    ...
    try {
      ...
      if (navigator.onLine !== false) await window.customerSupabase.auth.signOut();
  ...
  async function signIn(email, password) {
    ...
    if (navigator.onLine === false) {
      loading(false);
      message("لا يمكن تنفيذ تسجيل دخول جديد بدون اتصال. افتح التطبيق بالحساب المحفوظ سابقًا.");
      return;
    }
    try {
