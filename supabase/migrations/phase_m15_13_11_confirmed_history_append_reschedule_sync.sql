...
begin
  ...
  if jsonb_typeof(p_visits)<>'array' or jsonb_array_length(p_visits)<1 then raise exception 'يجب إضافة موعد جديد واحد على الأقل'; end if;
  ...
  if not v_has_confirmed and jsonb_array_length(p_visits)<2 then
    raise exception 'يجب إضافة يومين على الأقل لتقسيم الطلب';
  end if;
