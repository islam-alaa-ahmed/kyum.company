...
create policy "business activity report read" on public.business_activity_events for select to authenticated
using (
...
begin
  select representative_id into v_rep from public.user_profiles where id=auth.uid();
...
begin
  ...
  select representative_id into rep from public.user_profiles where id=uid;
