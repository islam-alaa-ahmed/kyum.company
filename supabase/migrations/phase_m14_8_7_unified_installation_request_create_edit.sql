...
begin
  ...
  if p_quotation_id is not null and not exists(
    ...
    where q.id=p_quotation_id and q.customer_id=p_customer_id
  ) then
  ...
  delete from public.installation_request_services
  ...
  where x.service_type_id is not null
    and x.quantity >= 1
    and x.unit_price >= 0;
...
-- The shared create/edit page must be viewable by every role that can edit installation requests.
insert into public.role_screen_permissions(
  role,screen_key,can_view,can_add,can_edit,can_delete,can_export
)
select p.role,'installationRequestNew',true,false,false,false,false
from public.role_screen_permissions p
where p.screen_key='installationRequests' and p.can_edit=true
